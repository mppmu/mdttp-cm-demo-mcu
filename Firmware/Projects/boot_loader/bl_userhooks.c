...
void BL_FwDownloadStart(void)
{
    ...
    GpioSet_LedMcuUser(g_ui8Led = LED_USER_RED_0 | LED_USER_RED_1);
}
...
void BL_FwDownloadProgress(void)
{
    ...
    g_ui8Led &= 0x3f;
    g_ui8Led++;
    GpioSet_LedMcuUser(g_ui8Led = LED_USER_RED_0 | LED_USER_RED_1 | g_ui8Led);
}
...
void BL_FwDownloadEnd(void)
{
    ...
    for (int i = 0; i < 4; i++) {
        GpioSet_LedMcuUser(g_ui8Led = 0xff);
        DelayUs(5e5);
        GpioSet_LedMcuUser(g_ui8Led = 0x00);
        DelayUs(5e5);
    }
}
